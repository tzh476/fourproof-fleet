...
|---|---:|---|---:|---|
| Innovation & Operational Utility | 40 | Specific enterprise onboarding gate; real catalog context; parallel autonomous evidence collection; code-enforced quarantine; canonical receipt | 32/40 | Show two non-fixture Gemini missions and a linked re-review across durable state |
| Architectural Discipline & Tech Stack | 30 | Google ADK fan-out/fan-in; one immutable evidence snapshot; Firestore transaction lease; Pub/Sub OIDC; SSRF/prompt-injection boundary; explicit failures; 32 backend tests | 27/30 | Prove the same graph, store, queue, lease, and failure behavior on Google Cloud |
...
| **Total** | **100** | **Strong local candidate; cloud proof is the critical path** | **72/100** | **Do not submit below 88/100** |
...
6. The public repo commit, Cloud Run revision, diagram, submission text, and video all describe the same architecture.
...
- **Video:** no continuous public four-minute video exists. It must show Google Cloud backend evidence.
